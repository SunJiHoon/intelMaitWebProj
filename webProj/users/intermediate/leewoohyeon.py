...
@router.get("/intermediate/leewoohyeon/calc", response_class=HTMLResponse)
async def get_calc_page(request: Request):
    return templates.TemplateResponse("intermediate/leewoohyeon/calc.html", {"request": request})

# imsoun's calc 페이지 라우팅 (POST 요청)
@router.post("/intermediate/leewoohyeon/calc", response_class=HTMLResponse)
async def post_calc_page(request: Request, num1: float = Form(...), num2: float = Form(...), operation: str = Form(...)):
    ...
    return templates.TemplateResponse("intermediate/leewoohyeon/calc.html", {"request": request, "result": result, "num1": num1, "num2": num2, "operation": operation})
